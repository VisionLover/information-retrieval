...
var app = new vue__WEBPACK_IMPORTED_MODULE_0__.default({
  ...
  mounted: function mounted() {},
  methods: {
    submit: function submit(pagenumber) {
      var _this = this;
      ...
      axios.post('http://127.0.0.1:8000/records', {
        ...
        _this.count = response.data[5];
        _this.paginate = response.data[6];

        if (length == 0) {
          if (_this.mysearch != "" && _this.executed == false) {
            $("#pq").fadeOut();
            ...
            setTimeout(function () {
              document.getElementById("search").style.top = "45px";
            }, 3500);
            setTimeout(function () {
              $('link[href="/css/notfound.css"]')[0].disabled = true;
            }, 4800);
            setTimeout(function () {
              $("#pq").fadeIn(1000);
            }, 4800);
          ...
          if (_this.mysearch != "" && _this.executed == true) {
            _this.items = [];

            _this.items.push({
              'title': "به‌نظر می‌رسد نتیجه خوبی مطابق با جستجوی شما وجود ندارد"
            });

            _this.items[0]['company'] = "نکته: سعی کنید از کلمه‌هایی استفاده کنید که ممکن است در صفحه‌ای که جستجو می‌کنید وجود داشته باشد. مثلاً به‌جای «دستور پخت کیک» از «نحوه درست کردن کیک» استفاده کنید.";
          }
        } else {
          if (_this.mysearch != "" && _this.executed == false) {
            _this.executed = true;
            ...
            for (var i = 0; i < length; i++) {
              ...
              _this.items[i]['link'] = response.data[1][i];
              _this.items[i]['city'] = response.data[2][i];
              _this.items[i]['company'] = response.data[3][i];
              _this.items[i]['time'] = response.data[4][i];
            }
          }
    ...
    suggest: function suggest(e) {
      ...
      axios.post('http://127.0.0.1:8000/suggest', {
        ...
        if (input == "" || _this2.executed == true) {
          _this2.sarrays = [];
        } else {
          _this2.sarrays = response.data;
        }
      });
    },
    close: function close(text) {
      $('#input').val(text);
      this.sarrays = [];
      this.mysearch = text;
    }
